# """
# Medallion Architecture scripts Pipeline Orchestrator.
...
# # from scripts.common import validate_post_load
...
#     download_dir: str = "datasets", skip_on_validation_fail: bool = False
# ) -> Dict[str, Any]:
#     """
#     Run the complete Medallion scripts pipeline.
...
#         skip_on_validation_fail: If True, abort scripts if staging validation fails
...
#         logger.info(
#             f"Bronze complete: {bronze_result['employees']} employees, "
...
#                     raise ValueError("scripts aborted due to staging validation failures")
#                 logger.warning("Proceeding with Gold load despite validation issues...")

#         logger.info(
#             f"Silver complete: {silver_result['employees']} employees, "
...
#             logger.info(
#                 f"Gold complete: {gold_result['dim_department']} departments, "
...
#         session = get_session()
#         try:
#             post_load_report = validate_post_load(
#                 session,
#                 {
...
#         logger.info("")
#         logger.info("=" * 70)
#         logger.info("scripts PIPELINE COMPLETED SUCCESSFULLY")
#         logger.info("=" * 70)
#         logger.info("")
#         logger.info("Summary:")
#         logger.info(
#             f"  Bronze: {bronze_result['employees']} employees, "
#             f"{bronze_result['timesheets']} timesheets"
#         )
#         logger.info(
#             f"  Silver: {silver_result['employees']} employees, "
#             f"{silver_result['timesheets']} timesheets (batch: {silver_result['batch_id']})"
#         )
#         if gold_result.get("status") == "success":
#             logger.info(
#                 f"  Gold: {gold_result['dim_department']} depts, "
...
# def run_etl_debug(
#     download_dir: str = "datasets", skip_on_qc_fail: bool = False
# ) -> Dict[str, Any]:
